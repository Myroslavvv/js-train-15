...
function generateRandomPassword(length) {
  ...
  for(let i=0; i < length; i++) {
    let res = characters.length;
    let random1 = Math.random() * res;
    let random2 = Math.floor(random1);
    let res2 = characters.charAt(random2);
    str += res2;
  }
  return str;
}
...
function roundObjectValues(obj) {
  ...
  let mas = Object.entries(obj);
  let res = mas.map(([key, value]) => {
...
function estimateInvestment(principal, interestRate, years) {
  ...
  let res1 = (principal) * Math.pow((1 + interestRate), years);
  let res2 = Math.round(res1);
  return res2;
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  let res1 = Math.fround(totalPrice);
  let res2 = Math.fround(maxPrice); 
  return (res1 > res2);
}
